...
| `github-actions-sa` | `roles/cloudfunctions.developer`<br>`roles/run.developer`<br>`roles/cloudbuild.builds.editor`<br>`roles/cloudscheduler.admin`<br>`roles/viewer`<br>`roles/artifactregistry.writer`<br>`roles/storage.admin`<br>`roles/eventarc.admin`<br>`roles/iam.serviceAccountUser` | GitHub Actions CI/CD deployment |
