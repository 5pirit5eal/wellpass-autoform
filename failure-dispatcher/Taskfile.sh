...
deploy() {
  ...
  local secret_id="${GITHUB_DISPATCH_SECRET_ID:-github-dispatch-token}"
  ...
  gcloud functions deploy "${func_name}" \
    ...
    --set-env-vars "PROJECT_ID=${project},GITHUB_OWNER=${owner},GITHUB_REPOSITORY=${repo},GITHUB_DISPATCH_SECRET_ID=${secret_id},GITHUB_EVENT_TYPE=cloud-run-job-failure" \
    --memory 256MB \
    --timeout 60s
}
