...
deploy-job() {
  ...
  local submitted_bucket="${SUBMITTED_BUCKET:-${source_bucket/processed/submitted}}"
  local failed_bucket="${FAILED_BUCKET:-${source_bucket/processed/failed}}"
  local typeform_url="${TYPEFORM_URL:-https://egym.typeform.com/to/z5XBrNXf}"
  local email="${EMAIL:?EMAIL is required}"
  local first_name="${FIRST_NAME:?FIRST_NAME is required}"
  local last_name="${LAST_NAME:?LAST_NAME is required}"
  local iban="${IBAN:?IBAN is required}"
  local bic="${BIC:?BIC is required}"
